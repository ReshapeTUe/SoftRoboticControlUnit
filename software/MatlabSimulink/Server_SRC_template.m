...
PiOutHeaderOrig.Variable = ["PresDes0"];                                 %actuators that you would like to control
PiOutHeaderOrig.Channel = [1];                                              %i2c channels that your actuators use for communication
PiOutHeaderOrig.VEAB = [1];                                                 %set to 1 for VEAB channels, and 0 for all other channels
PiOutHeaderOrig.Baseline = [0.49];                                          %baseline values [with scaling set to None!] that should be sent out when nothing is happening, and when the program is closed. So 0=-5V, and 1=+5V. For regulators, this is normally 0.5 (or 0.49 to avoid leakage)
PiOutHeaderOrig.StoringData = true;
...
PiInHeaderOrig.Channel = [1];                                               %i2c channels that your sensors use for communication
PiInHeaderOrig.VEAB = [1];                                                  %set to 1 for automatic VEAB sensor scaling, and 0 for all other channels
PiInHeaderOrig.StoringData = true;
...
[PiInHeader,PiInHeaderOrig,PiOutHeader,PiOutHeaderOrig,Pressure] = initializeStructs(PiInHeaderOrig,PiOutHeaderOrig,Pressure);
